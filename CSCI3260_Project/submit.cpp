...
Student Information
(0) Group Number: 33
(1) Student ID: 1155049055
(1) Student Name: Siu Long Wa
*********************************************************/
...
GLfloat animation_scaling = 6.0f;
GLfloat planet_wired = 1.0f;
int wired_pointer = 1;
...
void spaceshipInEarth() {
	if (spaceTravel == 0) {
		...
		spaceship_selfRotate += animation_scaling*spaceshipSpeed*0.0025f / 2.0f; //6.750131
		spaceship_Rotate += animation_scaling*spaceshipSpeed*0.1f / 2.0f; // 270.1
		...
		if (spaceTravelInProgress < 180) {
			spaceship_selfRotate -= animation_scaling*spaceshipSpeed*0.0005f; //6.750131
			spaceship_Rotate -= animation_scaling*spaceshipSpeed*0.02f; // 270.1
			spaceTravelInProgress += animation_scaling*spaceshipSpeed*0.02f;
		}
		else {
			spaceTravel = 0;
			spaceshipOnEarth = 0;
...
void spaceshipInPlanet() {
	if (spaceTravel == 0) {
		...
		rotMatrix2 = glm::rotate(glm::mat4(), glm::radians(45.0f), glm::vec3(0, 0, 1));
		transMatrix2 = glm::translate(glm::mat4(), glm::vec3(50.0f + zMouse, 2.0f + yMouse, 40.0f + xMouse));
		spaceship_selfRotate += animation_scaling*spaceshipSpeed*0.0025f; //6.750131
		spaceship_Rotate += animation_scaling*spaceshipSpeed*0.1f; // 270.1
		modelTransformMatrix = transMatrix2*rotMatrix2*AniRotMatrix2*transMatrix*AniRotMatrix*rotMatrix*scaleMatrix;
		if (spaceship_Rotate > 360.0) {
			spaceship_Rotate = 0;
			spaceship_selfRotate = 0;
...
void paintGL(void)  //always run
{
	...
	if (lightbox_required_movement > 100 && moveDirection == 1) {
		lightbox_required_movement -= animation_scaling*0.1;
		moveDirection = 0;
	}
	else
	...
	lightbox_selfRotate += animation_scaling*0.1;
	lightbox_rotate += animation_scaling*0.1;
	...
	earth_selfRotate += animation_scaling*0.01;
	// Avoid Overflow for long run
	...
	glDrawArrays(GL_TRIANGLES, 0, planet_size);
	moon_selfRotate += animation_scaling*0.0025;
	moon_Rotate += animation_scaling*0.1;
	// Avoid Overflow for long run
	if (moon_Rotate > 360) {
		moon_Rotate = 0;
	...
	glBindVertexArray(planet_vao);
	scaleMatrix = glm::scale(glm::mat4(), glm::vec3(planet_wired*4.0f, planet_wired*4.0f, planet_wired*4.0f));
	...
	tempLocation = glGetUniformLocation(generalID, "multipleTexture");
	glUniform1i(tempLocation, 1);
	tempLocation = glGetUniformLocation(generalID, "wiredMotion");
	glUniform1f(tempLocation, planet_wired);
	...
	planet_selfRotate += animation_scaling*1 / (planet_wired*planet_wired);
	if (planet_wired >= 1.0 && wired_pointer == 1)
	{
		planet_wired -= 0.001;
		wired_pointer = 0;
	}
	else
	if (planet_wired > 0.3 && wired_pointer == 1)
	{
		planet_wired += 0.001;
	}
	else
	if (planet_wired > 0.3 && wired_pointer == 0)
	{
		planet_wired -= 0.001;
	}
	else
	...
	rockRotate+= animation_scaling*0.05;
